 ...
 */
export function useRedirection<T>({
  uri,
  redirectIf,
  redirectTo,
  redirectConfig,
  ...
  const { data, error, mutate } = useSWR(uri, (url) =>
    fetch.get(url, { ...fetchConfig }).then((o) => o.json)
  );
  ...
  useLayoutEffect(() => {
    ...
    if (
      !redirectTo ||
      !redirectIf ||
      (typeof redirectIf == 'function' && !redirectIf((data || {}) as T))
    )
