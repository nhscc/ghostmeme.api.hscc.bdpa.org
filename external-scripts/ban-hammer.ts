...
export default async function main() {
  ...
  if (!maxRequestsPerWindow || !(Number(maxRequestsPerWindow) > 0)) {
    throw new AppError('BAN_HAMMER_MAX_REQUESTS_PER_WINDOW must be greater than zero');
  }

  if (!resolutionWindowSeconds || !(Number(resolutionWindowSeconds) > 0)) {
    throw new AppError('BAN_HAMMER_RESOLUTION_WINDOW_SECONDS must be greater than zero');
  }

  if (!defaultBanTimeMinutes || !(Number(defaultBanTimeMinutes) > 0)) {
    throw new AppError('BAN_HAMMER_DEFAULT_BAN_TIME_MINUTES must be greater than zero');
  }
